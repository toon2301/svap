...
describe('useSuggestions', () => {
  beforeEach(() => {
    // resetAllMocks aj tu: clearAllMocks zahodí len históriu volaní, kým
    // fronta z mockResolvedValueOnce ostane a prelieva sa do ďalšieho testu.
    jest.resetAllMocks();
...
describe('useSuggestions – prepnutie používateľa', () => {
  beforeEach(() => {
    // resetAllMocks, nie clearAllMocks: clear zahodí len históriu volaní, ale
    // mockResolvedValueOnce/mockImplementation z predošlého testu zostane
    // nastavená a „prekvapí" ďalší test. Predvolenú implementáciu preto
    // nastavujeme po resete nanovo.
    jest.resetAllMocks();
    (api.get as jest.Mock).mockResolvedValue({ data: { skills: [] } });
  });
...
describe('useSuggestions – zastarané odpovede', () => {
  beforeEach(() => {
    jest.resetAllMocks();
    (api.get as jest.Mock).mockResolvedValue({ data: { skills: [] } });
  });

  it('ignores a response that lands after the user has already switched', async () => {
    let releaseFirst: ((value: unknown) => void) | null = null;
    let releaseSecond: ((value: unknown) => void) | null = null;

    (api.get as jest.Mock)
      // Prvý request VISÍ – dobehne až po prepnutí používateľa.
      .mockImplementationOnce(
        () => new Promise((resolve) => {
    ...
    // Teraz dobehne odpoveď pre PREDOŠLÉHO používateľa.
    await act(async () => {
      releaseFirst?.({ data: { skills: [{ id: 1, title: 'Od prveho' }] } });
    });

    // Nesmie sa zobraziť – patrí používateľovi, ktorého už nepozeráme.
    expect(result.current.suggestedSkills).toEqual([]);

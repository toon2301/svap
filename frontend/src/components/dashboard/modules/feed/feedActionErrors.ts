...
export function translateFeedActionError(
  t: Translate,
  error: unknown,
  /** Vlastná hláška pre ostatné chyby (zdieľanie hlási niečo iné než lajk). */
  fallback?: () => string,
): string {
  const status = (error as { response?: { status?: number } })?.response?.status;
  if (status === 404 || status === 403 || status === 410) {
    return t(
      'feed.contentUnavailable',
      'Na tento obsah momentálne nie je možné reagovať.',
    );
  }
  return fallback ? fallback() : t('feed.likeError', 'Akciu sa nepodarilo uložiť.');
}

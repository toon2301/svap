...
export type DeleteSkillConfirmModalProps = {
  open: boolean;
  onClose: () => void;
...
export function DeleteSkillConfirmModal({
  open,
  onClose,
  onConfirm,
  isDeleting = false,
  ...
  return createPortal(
    <div
      ...
      aria-describedby="delete-skill-modal-desc"
        ...
        <div className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1 pt-0.5">
              <h2
                id="delete-skill-modal-title"
                className="text-lg font-semibold text-gray-900 dark:text-white"
              >
                {t('skills.deleteCardTitle', 'Vymazať kartu?')}
              </h2>
              <p
                id="delete-skill-modal-desc"
                className="mt-2 text-sm text-gray-600 dark:text-gray-400 leading-relaxed"
              >
                {t(
                  'skills.deleteCardMessage',
                  'Naozaj chceš vymazať túto kartu? Túto akciu nemožno vrátiť späť.',
                )}
              </p>
            </div>
          </div>

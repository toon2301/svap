...
def update_profile_view(request):
    ...
    original_data = {
        ...
        "phone_visible": request.user.phone_visible,
        "job_title": request.user.job_title,
        "job_title_visible": request.user.job_title_visible,
    ...
    if serializer.is_valid():
        ...
        for field, new_value in serializer.validated_data.items():
            if field in original_data and original_data[field] != new_value:
                changes[field] = {"old": original_data[field], "new": new_value}
        ...
        if (
            ...
            name_was_changed = (
                "first_name" in serializer.validated_data
                and serializer.validated_data.get("first_name")
                != original_data.get("first_name")
            ) or (
                "last_name" in serializer.validated_data
                and serializer.validated_data.get("last_name")
...
def get_draft_view(request, draft_type="general"):
    ...
    try:
        ...
        if draft_data:
            return Response(
                {"status": "draft_found", "data": draft_data, "draft_type": draft_type},
                status=status.HTTP_200_OK,
            )
        else:
            return Response(

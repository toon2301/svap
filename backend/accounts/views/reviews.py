...
def reviews_list_view(request, offer_id):
    ...
    if request.method == 'GET':
        ...
        if not is_owner and (offer.is_hidden or not getattr(offer.user, 'is_public', True)):
            return Response(
                {'error': 'Ponuka nebola nájdená'},
                status=status.HTTP_404_NOT_FOUND
            )
    ...
    if request.method == 'GET':
        ...
        reviews = Review.objects.filter(offer=offer).select_related('reviewer').order_by('-created_at')
...
def review_detail_view(request, review_id):
    ...
    if request.method == 'GET':
        ...
        if not offer_is_owner and (offer.is_hidden or not getattr(offer.user, 'is_public', True)):
            return Response(
                {'error': 'Recenzia nebola nájdená'},
                status=status.HTTP_404_NOT_FOUND
            )
    ...
    if request.method in ['PUT', 'PATCH']:
        ...
        review.delete()
        return Response(
            {'message': 'Recenzia bola odstránená'},

...
def notifications_list_view(request):
    ...
    return Response(NotificationSerializer(qs[:limit], many=True).data, status=status.HTTP_200_OK)
...
def notifications_unread_count_view(request):
    notif_type = (request.query_params.get('type') or NotificationType.SKILL_REQUEST).strip()
    try:
        count = Notification.objects.filter(user=request.user, type=notif_type, is_read=False).count()
    except Exception:
        count = 0
...
def notifications_mark_all_read_view(request):
    ...
    now = timezone.now()
    try:
        Notification.objects.filter(user=request.user, type=notif_type, is_read=False).update(is_read=True, read_at=now)
    except Exception:
        pass

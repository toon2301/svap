...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Bezpečnostný guard: nezvratné zmazanie VŠETKÝCH žiadostí nikdy nesmie
        # bežať v produkcii (DEBUG=False). --confirm flag nižšie ostáva ako
        # dodatočná vrstva ochrany.
        if not settings.DEBUG:
            raise CommandError(
                "delete_all_skill_requests je dostupný len v DEBUG režime (dev/test). "
                "V produkcii (DEBUG=False) je zakázaný – nezvratne maže všetky žiadosti."
            )

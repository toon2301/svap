...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if not options['confirm']:
            ...
            self.stdout.write(
                'Ak naozaj chceš pokračovať, spusti príkaz s --confirm:'
            )
            self.stdout.write(
                '  python manage.py delete_all_skill_requests --confirm'
            )
            return
        ...
        self.stdout.write(self.style.WARNING(
            f'Vymazávam {count} žiadostí z databázy...'
        ))
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(self.style.SUCCESS(
                    f'Vymazaných {deleted} žiadostí.'
                ))
        except Exception as e:
            self.stdout.write(self.style.ERROR(
                f'Chyba pri vymazávaní: {e}'
            ))
            raise

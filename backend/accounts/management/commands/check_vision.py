    ...
    def handle(self, *args, **options):
        try:
            ...
            if resp.error.message:
                self.stdout.write(
                    self.style.ERROR(f"Vision API error: {resp.error.message}")
                )
                return
            ...
            self.stdout.write(
                f"  SAFESEARCH_MIN_ADULT={getattr(settings, 'SAFESEARCH_MIN_ADULT', None)}"
            )
            self.stdout.write(
                f"  SAFESEARCH_MIN_VIOLENCE={getattr(settings, 'SAFESEARCH_MIN_VIOLENCE', None)}"
            )
            self.stdout.write(
                f"  SAFESEARCH_MIN_RACY={getattr(settings, 'SAFESEARCH_MIN_RACY', None)}"
            )
            ...
            self.stdout.write(
                f"DEBUG has_json={has_json} STRICT_MODE={getattr(settings, 'SAFESEARCH_STRICT_MODE', False)}"
            )
            ...
            try:
                ...
                exists = False
            self.stdout.write(
                f"DEBUG GOOGLE_APPLICATION_CREDENTIALS set={bool(path)} exists={exists}"
            )

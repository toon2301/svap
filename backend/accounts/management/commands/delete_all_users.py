...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if user_count == 0:
            self.stdout.write(self.style.SUCCESS('V databaze nie su ziadni pouzivatelia.'))
            return

        self.stdout.write(self.style.WARNING(
            f'Vymazavam {user_count} pouzivatelov z databazy...'
        ))
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(
                f'Chyba pri vymazavani pouzivatelov: {e}'
            ))
            raise

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        site, created = Site.objects.get_or_create(
            id=1, defaults={"domain": domain, "name": "Swaply"}
        )

        if created:
            self.stdout.write(self.style.SUCCESS(f"Vytvorený Site: {site.domain}"))
        ...
        if not created:
            ...
            social_app.save()
            self.stdout.write(self.style.SUCCESS("Google SocialApp bol aktualizovaný"))
        else:
            self.stdout.write(self.style.SUCCESS("Google SocialApp bol vytvorený"))

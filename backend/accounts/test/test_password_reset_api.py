...
class TestPasswordResetAPI(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='resetuser', email='reset@example.com', password='StrongPass123', is_verified=True
        )
    ...
    def test_password_reset_confirm_invalid_uid(self):
        url = reverse('accounts:password_reset_confirm', kwargs={'uidb64': '!!', 'token': 'abc'})
    ...
    def test_password_reset_verify_token_invalid(self):
        url = reverse('accounts:password_reset_verify', kwargs={'uidb64': '!!', 'token': 'abc'})
    ...
    def test_password_reset_verify_token_valid(self):
        uid = urlsafe_base64_encode(force_bytes(self.user.pk))
        token = default_token_generator.make_token(self.user)
        url = reverse('accounts:password_reset_verify', kwargs={'uidb64': uid, 'token': token})
        r = self.client.get(url)
        assert r.status_code == status.HTTP_200_OK

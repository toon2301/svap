...
class TestRedisFallback(TestCase):
    ...
    def test_redis_unavailable_fallback(self):
        with patch(
            "accounts.authentication.cache.get",
    ...
    def test_token_blacklist_redis_fallback(self):
        ...
        with patch(
            ...
            with patch(
                "accounts.authentication.cache.set",
                side_effect=Exception("Redis unavailable"),
            ):
                refresh.blacklist()
    ...
    def test_token_blacklist_redis_available(self):
        ...
        with patch("accounts.authentication.cache.get", return_value=None):
            with patch.object(
                SwaplyRefreshToken, "_is_redis_available", return_value=True
            ):
                with patch(
                    "accounts.authentication.cache.set", return_value=True
                ) as mock_set:
                    refresh.blacklist()

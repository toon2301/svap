...
class TestAuthEndpoints(APITestCase):
    ...
    def test_register_post_success(self, mock_send_mail):
        ...
        payload = {
            ...
            'user_type': 'individual', 'birth_day': '01', 'birth_month': '01', 'birth_year': '2000', 'gender': 'male',
    ...
    def test_me_with_tokens(self):
        user = User.objects.create_user(username='meu', email='me@example.com', password='StrongPass123', is_verified=True)
    ...
    def test_logout_with_refresh(self):
        user = User.objects.create_user(username='lou', email='lou@example.com', password='StrongPass123', is_verified=True)
        self.client.force_authenticate(user=user)
        ...
        r2 = self.client.post(refresh_token_url, {'refresh': str(refresh)}, format='json')
        assert r2.status_code in (status.HTTP_401_UNAUTHORIZED, status.HTTP_400_BAD_REQUEST)

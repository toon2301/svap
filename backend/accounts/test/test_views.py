...
class TestAuthViews(APITestCase):
    ...
    def test_login_view_success(self):
        ...
        self.assertIn("X-Svaply-Access-Expires-At", response.headers)
        self.assertIn("X-Svaply-Access-Expires-In", response.headers)
    ...
    def test_me_view_exposes_server_timing_breakdown(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        server_timing = response.headers.get("Server-Timing", "")
        self.assertIn("auth_user_cache_get", server_timing)
        self.assertIn("auth_user_cache_hit", server_timing)
        self.assertIn("auth_user_cache_miss", server_timing)
        self.assertIn("auth_user_db_connect", server_timing)
        self.assertIn("auth_user_db_query", server_timing)
        self.assertIn("auth_user_db_get", server_timing)
        self.assertIn("auth_user_cache_set", server_timing)
        self.assertIn("auth_user_cache_set_skipped", server_timing)

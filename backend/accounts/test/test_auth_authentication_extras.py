...
class TestJWTAuthExtras:
    ...
    def test_get_user_inactive_user_raises(self):
        user = User.objects.create_user(username='inactive', email='inactive@example.com', password='StrongPass123', is_active=False)
        token = {'user_id': user.id, 'jti': 'x1'}
        auth = SwaplyJWTAuthentication()
        # Zabezpeč, že nevyhodí blacklist pred overením usera
        with patch.object(SwaplyJWTAuthentication, '_is_redis_available', return_value=False):
    ...
    def test_fallback_blacklisted_token_raises(self):
        user = User.objects.create_user(username='bluser', email='bl@example.com', password='StrongPass123', is_active=True)
    ...
    def test_redis_available_not_blacklisted_returns_user(self):
        user = User.objects.create_user(username='okuser', email='ok@example.com', password='StrongPass123', is_active=True)
        token = {'user_id': user.id, 'jti': 'ok123'}
        auth = SwaplyJWTAuthentication()
        with patch.object(SwaplyJWTAuthentication, '_is_redis_available', return_value=True):
            with patch('django.core.cache.cache.get', return_value=None):
    ...
    def test_redis_blacklist_check_exception_path_returns_user(self):
        user = User.objects.create_user(username='rerr', email='rerr@example.com', password='StrongPass123', is_active=True)
    ...
    def test_refresh_blacklist_logs_base_failure_and_uses_fallback(self, caplog):
        user = User.objects.create_user(username='rb', email='rb@example.com', password='StrongPass123', is_active=True)
        refresh = SwaplyRefreshToken.for_user(user)
        with patch('accounts.authentication.RefreshToken.blacklist', side_effect=Exception('base-fail')):
            with patch.object(SwaplyRefreshToken, '_is_redis_available', return_value=False):
                with patch('django.core.cache.cache.set', return_value=True) as mock_set:
                    refresh.blacklist()
                    assert any('Base blacklist failed' in rec.message for rec in caplog.records)
    ...
    def test_refresh_blacklist_logs_error_on_cache_failure(self):
        user = User.objects.create_user(username='rb2', email='rb2@example.com', password='StrongPass123', is_active=True)
        refresh = SwaplyRefreshToken.for_user(user)
        with patch.object(SwaplyRefreshToken, '_is_redis_available', return_value=True):
            with patch('accounts.authentication.cache.set', side_effect=Exception('cache-fail')):
                # Should not raise
                refresh.blacklist()
    ...
    def test_redis_available_blacklisted_raises(self):
        user = User.objects.create_user(username='rb3', email='rb3@example.com', password='StrongPass123', is_active=True)

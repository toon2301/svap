...
class TestAuthMisc(APITestCase):
    ...
    def test_google_oauth_success_and_bad_token(self, mock_get, mock_post):
        ...
        callback_url = reverse('accounts:google_callback') + '?code=bad&state=http://localhost:3000/auth/callback/'
        r_cb_bad = self.client.get(callback_url)
        assert r_cb_bad.status_code in (302, 200)

        # 2) Success exchange
        mock_post.return_value = Mock(status_code=200, json=lambda: {'access_token': 'at'})
        mock_get.return_value = Mock(status_code=200, json=lambda: {'email': 'oauth@example.com', 'given_name': 'O', 'family_name': 'Auth'})
        callback_url_ok = reverse('accounts:google_callback') + '?code=ok&state=http://localhost:3000/auth/callback/'

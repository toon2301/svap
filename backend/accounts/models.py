...
class Review(models.Model):
    ...
    def clean(self):
        ...
        if float(self.rating) % 0.5 != 0:
            raise ValidationError({'rating': 'Hodnotenie musí byť v krokoch 0.5 (napr. 3.5, 4.0).'})
        ...
        for i, pro in enumerate(self.pros):
            ...
            if len(pro) > 120:
                raise ValidationError({'pros': f'Plus #{i+1} môže mať maximálne 120 znakov.'})
        
        for i, con in enumerate(self.cons):
            if not isinstance(con, str):
                raise ValidationError({'cons': f'Mínus #{i+1} musí byť text.'})
            if len(con) > 120:
                raise ValidationError({'cons': f'Mínus #{i+1} môže mať maximálne 120 znakov.'})
        
        # Text má max 300 znakov
        if len(self.text) > 300:
            raise ValidationError({'text': 'Text recenzie môže mať maximálne 300 znakov.'})
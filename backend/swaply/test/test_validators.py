...
class TestEmailValidator:
    ...
    @pytest.mark.parametrize(
        "email", ["", "bad", "a@b", "x@", "x@z.", "a" * 255 + "@example.com"]
    )
...
class TestPasswordValidator:
    ...
    @pytest.mark.parametrize(
        "password", ["nouppercase1", "NOLOWERCASE1", "NoDigitsAAAA"]
    )
...
class TestURLValidator:
    ...
    def test_valid(self):
        assert (
            URLValidator.validate_url("https://example.com", "Web")
            == "https://example.com"
        )

    @pytest.mark.parametrize(
        "url", ["ftp://x", "example.com", "http://" + "a" * 205 + ".com"]
    )

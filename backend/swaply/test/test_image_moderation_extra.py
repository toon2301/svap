...
@pytest.mark.skip(
    reason="Env JSON branch depends on Google crypt signer; covered by other paths."
)
def test_get_client_prefers_env_json(monkeypatch, settings):
    ...
    monkeypatch.setitem(
        ...
        type("svc", (), {"Credentials": DummyCred}),
    )
...
def test_get_client_file_path(monkeypatch, settings, tmp_path):
    ...
    p = tmp_path / "svc.json"
    p.write_text(
        '{"client_email":"f@y","private_key":"k","private_key_id":"id","type":"service_account","token_uri":"https://oauth2.googleapis.com/token"}',
        encoding="utf-8",
    )
...
@pytest.mark.skip(
    reason="ADC fallback relies on google.auth default env; skip in unit tests."
)

...
def _get_client():
    ...
    if creds_json:
        ...
        if getattr(settings, "SAFESEARCH_STRICT_MODE", False):
            raise RuntimeError(
                "Vision credentials not configured (no env JSON nor GOOGLE_APPLICATION_CREDENTIALS file)"
            )
...
def _violates_thresholds(
    safe_search_result, thresholds: Dict[str, str]
) -> Dict[str, str]:
    ...
    for key in ("adult", "violence"):
        ...
        if actual_int >= min_int:
            name = (
                LIKELIHOOD_NAMES[actual_int]
                if 0 <= actual_int < len(LIKELIHOOD_NAMES)
                else "UNKNOWN"
            )
            offending[key] = name
    ...
    if racy_min_level is not None:
        ...
        block_on_racy_alone = getattr(
            settings, "SAFESEARCH_BLOCK_ON_RACY_WITHOUT_ADULT", False
        )
        # Blokuj racy iba ak je to vynútené, alebo ak už blokujú adult/violence
        if actual_racy >= racy_min_int and (
            block_on_racy_alone or "adult" in offending or "violence" in offending
        ):
            name = (
                LIKELIHOOD_NAMES[actual_racy]
                if 0 <= actual_racy < len(LIKELIHOOD_NAMES)
                else "UNKNOWN"
            )
            offending["racy"] = name
    return offending
